...
import type { CardColors } from "@/types/enum/card-colors.enum";
...
<template>
  <el-drawer v-model="open" title="Adicionar cartao" :direction="direction" :before-close="handleClose">
      ...
      <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col items-center gap-4">
          <div class="flex items-center gap-2">
            <el-button v-for="color in cardsFormatted" :key="color.key"
              @click="onChangeCardColor(color.key as CardColors)"
              :class="[color.value, color.key === selectedCardColor && 'border-2! border-zinc-800!', 'w-5! h-5!']"
              circle></el-button>
          </div>
          <div :class="[cardColors[selectedCardColor], 'w-full flex p-3 transition-all rounded-xl']">
            ...
            <div class="flex flex-col gap-2 h-48 justify-end">
              <el-text class="text-white! font-semibold!">{{ values.name.length === 0 ? "Digite um nome" : values.name
              }}</el-text>
              <div>
                <el-text class="text-white! font-semibold!">{{ values.expiration.length === 0 ? "00/00" :
                  format(values.expiration, "MM/yy") }}</el-text>
              </div>
            </div>
          </div>
        </div>
    ...
    <template #footer>
      <el-button @click="onSubmit" class="w-fit p-5! flex! justify-center font-semibold!" type="primary"
        round>Salvar</el-button>
    </template>
  </el-drawer>
